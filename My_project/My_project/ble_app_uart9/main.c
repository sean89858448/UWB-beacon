...
static void advertising_update(void)
{
    ...
    update2_beacon_info[7] = saadc_val;
    advdata.name_type             = BLE_ADVDATA_NO_NAME;
    advdata.flags                 = flags;
    advdata.p_manuf_specific_data = &manuf_specific_data;
    
    err_code = ble_advdata_encode(&advdata, m_adv_data.adv_data.p_data, &m_adv_data.adv_data.len);
    if APP_ERROR_CHECK(err_code);
}
...
static int AT_command_judge(uint8_t *print_value, uint16_t timeout)
{
...
int main(void)
{
    ...
    nrf_delay_ms(2000);
    printf("AT+anchor_tag=0,1\r\n");
    nrf_delay_ms(500);
    printf("AT+interval=10\r\n");
    nrf_delay_ms(500);
    printf("AT+RST\r\n");
    nrf_delay_ms(800);
    printf("AT+switchdis=1\r\n");
    nrf_delay_ms(2000);
                ...
                if(cr == '\n') {
                    ...
                    if((cmd[0] == 'a') && (cmd[1] == 'n')) {
                        ...
                        if(ID_Index == 3) {
                            ...
                            //UUID自訂義用來辨識
                            update2_beacon_info[2] = 0xdb, update2_beacon_info[3] = 0x15, update2_beacon_info[4] = 0x17,update2_beacon_info[5] = 0x14, update2_beacon_info[6] = 0x28;
                            //UUID的　　ID、 距離
